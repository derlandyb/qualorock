...
- **Phase / Task**: `infrastructure` is fully Executed and Verified. `admin-panel` Execute is now underway: Phase 1 (T1-T7), Phase 2 (T8-T10), Phase 3 (T11-T14), and Phase 4 (Venue & promoter management, T15-T16) are all merged to `qualorock-api`'s `main` (PR #1, #2, #3, #4) and the root repo's `api` submodule pointer is updated to match. Phase 4 shipped `VenueController` (show/update/agenda/history — no create/delete, a deliberate scope decision since spec ADMIN-13/14's ACs only cover update + two reads and `venues.organizer_id` is DB-unique) and `PromoterController` (register/edit/remove + link/unlink to events, with edits/removals propagating since promoter data isn't denormalized per-event; promoter delete detaches `event_promoter` pivot rows before deleting since the pivot's FK defaults to RESTRICT, not CASCADE). `LinkPromoterRequest`/`UnlinkPromoterRequest` are the first FormRequests in this codebase checking ownership of two separate resources (promoter + event) in one `authorize()`; `LinkPromoterRequest` now extends `OrganizerOwnedPromoterRequest` rather than duplicating its resolution logic, and a shared `EventEntityMapper` replaced the duplicated Eloquent→domain Event mapping that briefly existed in both `EloquentEventRepository` and `EloquentVenueRepository` — both fixed in a follow-up commit after the automated PR review flagged them. Phases 5-15 of `admin-panel/tasks.md` have not started. `web-app`, `landing-page-plans`, `mobile-app` still have Specify/Design/Tasks complete but Execute not started. `dev-logging`/`analytics-tracking` remain Specify-only.
- **Completed**: This session - admin-panel Phase 4 (T15-T16) implemented, tested, and independently Verified (PASS, single pass, no fix→re-verify loop needed): 7/7 ACs (ADMIN-13/14/17/18/19) plus the promoter-deletion edge case matched spec-defined outcomes exactly (1 non-blocking spec-precision/scope note — AC2's "consumer-facing" wording is tested at the organizer-facing data layer this repo owns; actual consumer rendering is web-app/mobile-app's scope), 5/5 discrimination-sensor mutations killed, 49/49 tests passing (37→49, +12, no regressions). Report at `.specs/features/admin-panel/validation.md`. `design.md`'s thin `VenueController, PromoterController` entry filled in with an Interfaces route list, Dependencies line, the venue-scope decision, and a layering note; `tasks.md`'s T15/T16 checkboxes marked complete. PR #4 opened on `qualorock-api`, reviewed by the local automated code-review pass — 0 correctness bugs, 2 reuse/cleanup findings (duplicated promoter-resolution logic in `LinkPromoterRequest`; duplicated Event entity mapping between `EloquentEventRepository`/`EloquentVenueRepository`) — both fixed in a follow-up commit (`refactor(admin-panel): dedupe promoter request and event entity mapping`), re-verified green (49/49 tests, Pint clean). CI green (lint/test/quality-gate), PR merged via squash, submodule pointer bumped.
- **In-progress**: None - Phase 4 is fully closed out (implemented, verified, reviewed, fixed, merged, submodule bumped).
- **Next step**: Phase 5 (Engagement dashboard - T17) is next in `admin-panel/tasks.md`. Unresolved items carried forward from Phase 3, still open: (a) the CI test job still has no coverage `--min` threshold - revisit once later phases add enough tested code; (b) `EloquentOrganizerRepository::update(int $id, array $attributes)`'s generic attributes-array signature remains a latent mass-assignment/IDOR risk if a future endpoint forwards user-supplied fields through it unfiltered; (c) candidate lesson `L-004` (duplicate/copy field-equivalence tests should enumerate every field the use-case's constructor call actually assigns) - still applies to future duplicate/copy tests, not exercised by Phase 4 (no duplicate/copy use-case here).
- **Blockers**: none currently open.
- **Uncommitted files**: none - this Handoff update is committed alongside the submodule-pointer bump.
- **Branch**: `main` is the default branch on both the root repo and `qualorock-api`; `feat/admin-panel-phase-4-venue-promoter` was deleted on merge (GitHub auto-delete) and its local copy removed.
