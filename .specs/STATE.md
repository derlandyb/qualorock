...
- **Phase / Task**: `infrastructure` is fully Executed and Verified. `admin-panel` Execute is underway: Phase 1 (T1-T7), Phase 2 (T8-T10), Phase 3 (T11-T14), Phase 4 (Venue & promoter management, T15-T16), Phase 5 T19 (`EventInfoRequestController`, ADMIN-16), and Phase 6 T20 (`PlanPricingController`, ADMIN-20..23) are merged to `qualorock-api`'s `main` (PR #1-#6) and the root repo's `api` submodule pointer matches. Phase 4 shipped `VenueController` (show/update/agenda/history — no create/delete, deliberate scope decision) and `PromoterController` (register/edit/remove + link/unlink, with edits/removals propagating). Phase 5 is **partially done**: T19 is merged; T17 (`EngagementDashboardController`) and T18 (`AudienceInterestController`) remain deliberately deferred — both depend on `event_stats`/`Favorite`/`Friendship`/`EventInterest` tables that `web-app`'s design.md owns and whose Execute hasn't started. Phase 6 is fully done (its only task, T20, is merged). Phases 7-15 of `admin-panel/tasks.md` have not started. `web-app`, `landing-page-plans`, `mobile-app` still have Specify/Design/Tasks complete but Execute not started. `dev-logging`/`analytics-tracking` remain Specify-only.
- **Completed**: This session - implemented and merged PR #6 (admin-panel T20, `PlanPricingController`) into `qualorock-api` main via `gh pr merge --merge` (merge commit `4a41307`). `PlanPriceRepositoryInterface` extended with `history()` and an atomic `setNewCurrent()`; `PlanPricingPolicy`, `GetPlanPriceHistory`/`SetPlanPrice` use-cases, `SetPlanPriceRequest`, `PlanPricingController` (`GET`/`POST /super-admin/plan-prices`), routes, DI binding, factory, and feature tests added, mirroring `OrganizerApprovalController`'s guard-less-route + policy + `abort_unless`/`FormRequest::authorize()` pattern (super_admin-only, 403 not 401 for wrong/no guard). Independently Verified (PASS): 4/4 spec ACs and all 3 literal T20 Done-when criteria covered with file:line evidence; discrimination sensor found 3/5 mutations killed on first pass, the 2 survivors (append-only-guard coverage, history-ordering coverage) closed same-session. A subsequent code-review pass on PR #6 found 2 more should-fix gaps beyond the sensor's scope — `setNewCurrent()` wasn't safe under concurrent requests (two callers could each leave an "open" row for the tier), and `amount` had no upper bound (integer overflow → 500 instead of 422) — both closed before merge: a new migration adds a partial unique index (`UNIQUE (tier) WHERE effective_to IS NULL`) enforcing the invariant at the DB level, `setNewCurrent()`'s close-update now takes `lockForUpdate()`, and a new `AdminPanelConstants::PLAN_PRICE_MAX_CENTS` ceiling is validated by `SetPlanPriceRequest`. Root repo's `api` submodule pointer bumped to the merged commit; `tasks.md`'s T20 checkboxes and `spec.md`'s ADMIN-20..23 traceability rows marked done; `validation.md` updated with the full Verifier + post-verification review report.
- **In-progress**: none for T20 — it is merged, submodule-bumped, and closed out.
- **Next step**: Commit this Handoff update plus the bumped `api` submodule pointer to the root repo. Phase 7 of `admin-panel/tasks.md` has not started. T17/T18 stay blocked until `web-app`'s Execute reaches its data-model tasks (or the user decides to build the shared tables under a different feature now). Unresolved items carried forward: (a) CI test job still has no coverage `--min` threshold; (b) `EloquentOrganizerRepository::update()`'s generic attributes-array signature remains a latent mass-assignment/IDOR risk if a future endpoint forwards unfiltered user input through it; (c) candidate lesson `L-004` (duplicate/copy field-equivalence tests should enumerate every field the use-case's constructor call actually assigns) - not exercised this session either; (d) `spec.md`'s traceability table for ADMIN-01..15/17..19/24..27 remains stale (pre-existing gap, not touched this session).
- **Blockers**: T17/T18 blocked on `web-app` Execute reaching its `event_stats`/`Favorite`/`Friendship`/`EventInterest` data model (see Phase/Task above). Not a blocker on T20, which is done. No blockers on Phase 7+.
...
- **Branch**: root repo `main`. `api` submodule is on `main`, up to date with `qualorock-api`'s `main` (includes the Phase 6 T20 merge).
