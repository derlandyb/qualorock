...
| ---------- | ------------------- | --------------------- | ----------------- | ----------- |
| Dockerfile (per service) | none | - (build gate only) | backend/Dockerfile, website/Dockerfile, adminpanel/Dockerfile, landingpage/Dockerfile | docker build . |
...
| .env.example templates | none | - (build gate only): documents every var the compose file references | backend/.env.example, website/.env.example, adminpanel/.env.example, landingpage/.env.example | docker compose config -q |
...
| Makefile | none | - (build gate only): `up` boots every default-profile service; `test-e2e` runs Playwright via the test profile only | Makefile | make up && docker compose ps |
| DatabaseSeeder run order + Makefile `seed` target | none | - (build gate only): runs admin-panel's then web-app's seeders in FK-safe order with zero errors | backend/database/seeders/DatabaseSeeder.php, Makefile | make seed |
...
### T1: Create backend Dockerfile (PHP 8.4 + Laravel)
...
- [ ] Dockerfile builds a PHP 8.4 image with Composer dependencies installed
- [ ] Image exposes the app port and can run `php artisan serve` or php-fpm
- [ ] Image can also run `php artisan reverb:start` for the websocket process
- [ ] `docker build -f backend/Dockerfile .` succeeds with exit code 0

**Tests**: none
**Gate**: build

**Commit**: `chore(infra): add backend Dockerfile`
...
- [ ] `docker build -f website/Dockerfile .` succeeds with exit code 0
...
### T3: Create admin-panel Dockerfile (React/Node dev server)

**What**: Dockerfile for the `adminpanel` submodule's React+Tailwind dev server, mirroring T2's pattern.
**Where**: `adminpanel/Dockerfile`
**Depends on**: None
...
- [ ] `docker build -f adminpanel/Dockerfile .` succeeds with exit code 0

**Tests**: none
**Gate**: build

**Commit**: `chore(infra): add admin-panel Dockerfile`
...
- [ ] `docker build -f landingpage/Dockerfile .` succeeds with exit code 0
...
**What**: Root `docker-compose.yml` defining services: `backend` (T1 image), `reverb` (same image, different command), `postgres` (least-privilege app user per AD-004/AD-008), `minio` (S3-compatible store per AD-009), `mailhog` (SMTP catcher per AD-004's Laravel Mail decision), `web-app` (T2), `admin-panel` (T3), `landing-page-plans` (T4) - with named volumes for Postgres/MinIO data and a shared network.
...
- [ ] All eight services declared with correct build contexts (T1-T4 Dockerfiles) or pinned images (postgres, minio, mailhog)
...
**What**: One `.env.example` per service (backend, website, adminpanel, landingpage) documenting every environment variable docker-compose.yml references (DB DSN, Sanctum/session domain, Reverb keys, MinIO/S3 credentials, Mailhog SMTP host/port, Google Maps key placeholder, OAuth client-id placeholders).
**Where**: `backend/.env.example, website/.env.example, adminpanel/.env.example, landingpage/.env.example`
**Depends on**: T5
...
- [ ] Every variable referenced in docker-compose.yml's `environment:`/`env_file:` blocks has a matching placeholder line in the relevant .env.example
...
### T9: Add root Makefile (up/down/logs/ps/build/test-e2e)

**What**: Root `Makefile` wrapping the compose commands so no developer needs to know the raw invocation: `up` (`docker compose up -d --wait` - every default-profile service: backend, reverb, postgres, minio, mailhog, web-app, admin-panel, landing-page-plans), `down` (`docker compose down`), `logs` (`docker compose logs -f`), `ps` (`docker compose ps`), `build` (`docker compose build`), and `test-e2e` (`docker compose --profile test run --rm playwright ...`, the ONLY target that starts Playwright).
**Where**: `Makefile`
**Depends on**: T8
...
- [ ] GIVEN `make up` WHEN run on a clean checkout THEN backend, reverb, postgres, minio, mailhog, web-app, admin-panel, and landing-page-plans all reach Up/healthy - test passes
...
- [ ] GIVEN `make test-e2e` WHEN run THEN the `playwright` container starts via the `test` profile, runs, and exits with the suite's real pass/fail code
- [ ] `make down` cleanly stops and removes all containers started by `make up`

**Tests**: none
**Gate**: build

**Commit**: `chore(infra): add root Makefile with up/down/logs/ps/build/test-e2e targets`

---

### T10: Smoke-test: full containerized stack boots healthy

**What**: Bring up every containerized service and confirm each reaches a running/healthy state, proving T1-T7 compose into a working dev environment before any feature work starts on top of it.
**Where**: `docker-compose.yml`
**Depends on**: T5, T6, T7, T8, T9
**Requirement**: INFRA-08, INFRA-09, INFRA-10
...
- [ ] `docker compose up -d --wait` exits 0 (all services with healthchecks report healthy; others report running)
- [ ] `docker compose ps` shows backend, reverb, postgres, minio, mailhog, web-app, admin-panel, landing-page-plans all Up
- [ ] `docker compose ps` does NOT show a `playwright` container after this plain `docker compose up` (profile isolation actually verified, not just declared in T9)
- [ ] `make up` produces the identical result as the raw `docker compose up -d --wait` command above
- [ ] `docker compose down` cleanly stops and removes all containers

**Tests**: none
**Gate**: build

**Commit**: `chore(infra): verify full containerized dev stack boots healthy`
...
**Where**: `backend/database/seeders/DatabaseSeeder.php, Makefile`
**Depends on**: T9
...
- [ ] `make seed` runs `AdminPanelSeeder` before `WebAppSeeder` with zero foreign-key errors on a freshly migrated database
- [ ] Running `make seed` twice in a row does not fail or duplicate rows (both underlying seeders are idempotent per their own tasks)
- [ ] `make up && make seed` succeeds end-to-end on a clean checkout
...
- [ ] The collection contains one folder per feature (admin-panel, web-app, landing-page-plans) with one request per implemented endpoint
- [ ] Every request that requires auth includes a working seeded credential/token in the environment file, not a placeholder
- [ ] Every request body/param references a real seeded record ID from T11's fixtures where applicable (e.g. a real event ID for `GET /consumer/events/{id}`)
- [ ] GIVEN the `make up` stack is running and `make seed` has been run THEN `newman run postman/QualORock.postman_collection.json -e postman/local.postman_environment.json --folder smoke` (a curated smoke subset, one request per feature) exits 0
...
**What**: Markdown write-up of the local dev environment: every service in docker-compose.yml, the Makefile targets (`up`/`down`/`logs`/`ps`/`build`/`test-e2e`/`seed`), the host-vs-container split (mobile-app on host), and the Playwright test-only profile - restates `docs/development.md`'s content in the same per-stack documentation location as every other feature, per AD-014.
...
- [ ] Doc lists every compose service and its purpose
- [ ] Doc lists every Makefile target and what it does
...
| Task | Scope | Status |
| ---- | ----- | ------ |
| T1: Create backend Dockerfile (PHP 8.4 + Laravel) | 1 file | ✅ Granular |
| T2: Create web-app Dockerfile (React/Node dev server) | 1 file | ✅ Granular |
| T3: Create admin-panel Dockerfile (React/Node dev server) | 1 file | ✅ Granular |
...
| T9: Add root Makefile (up/down/logs/ps/build/test-e2e) | 1 file | ✅ Granular |
...
| T8 | T5 | T5 | ✅ Match |
| T9 | T8 | T8 | ✅ Match |
| T10 | T5, T6, T7, T8, T9 | T5, T6, T7, T8, T9 | ✅ Match |
| T11 | T9 | T9 | ✅ Match |
| T12 | T11 | T11 | ✅ Match |
...
| ---- | ---------------------------- | ---------------- | ---------- | ------ |
| T1: Create backend Dockerfile (PHP 8.4 + Laravel) | Dockerfile (per service) | none | none | ✅ OK |
| T2: Create web-app Dockerfile (React/Node dev server) | Dockerfile (per service) | none | none | ✅ OK |
| T3: Create admin-panel Dockerfile (React/Node dev server) | Dockerfile (per service) | none | none | ✅ OK |
...
| T9: Add root Makefile (up/down/logs/ps/build/test-e2e) | Makefile | none | none | ✅ OK |
