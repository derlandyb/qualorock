...
- **Interfaces**: `GET /organizer/events`, `POST /organizer/events`, `PUT /organizer/events/{id}`, `POST /organizer/events/{id}/duplicate`, `DELETE /organizer/events/{id}`, `POST /organizer/events/{id}/status`
...
- **Layering note**: `Presentation/Http/Requests/Organizer/OrganizerOwnedEventRequest.php` (the shared base class behind `Update`/`Delete`/`Duplicate`/`TransitionEventStatus`'s FormRequests) resolves `EventRepositoryInterface` directly to load the route-bound event for its `authorize()` ownership check, rather than routing through an Application use-case. This wasn't specced here originally (added during Phase 3 Execute). The deviation is deliberate, reviewed, and left as-is: a FormRequest's `authorize()` runs before any use-case is invoked, so there is no use-case to call at that point in the request lifecycle without introducing a read-only "fetch event" use-case whose only caller would be this one ownership check - the same reasoning Phase 2's `LoginOrganizer` deviation already established for this codebase (framework-lifecycle-coupled reads that don't carry business logic are not forced through the Domain repository contract's use-case wrapper).
